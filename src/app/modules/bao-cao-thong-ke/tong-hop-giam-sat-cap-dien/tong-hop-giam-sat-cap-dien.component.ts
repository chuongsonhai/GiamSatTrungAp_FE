...
@Component({
  ...
  templateUrl: './tong-hop-giam-sat-cap-dien.component.html',
  styleUrls: ['./tong-hop-giam-sat-cap-dien.component.scss'],
  providers: [
...
export class TongHopGiamSatCapDienComponent implements
OnInit,
OnDestroy,
IDeleteAction,
IDeleteSelectedAction,
...
filter() {
  ...
  const TenLoaiCanhBao = this.filterGroup.controls['TenLoaiCanhBao'].value;
  if (TenLoaiCanhBao) {
    filter['TenLoaiCanhBao'] = TenLoaiCanhBao;
  }
  ...
  this.service.getTHGiamSatCapDien({Filterbctd:filter}).pipe(
    catchError(err => {
      return of(undefined);
    })).subscribe((response) => {
...
exportExcel() {
  ...
  this.service.exportExcelTHGiamSatCapDien({Filterbctd:filter}).subscribe((response) => {
    if(response === undefined || response === null){
      ...
      link.download="TongHopGiamSatCapDien.xlsx";
      link.click();
