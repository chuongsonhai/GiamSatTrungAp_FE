...
@Component({
  selector: 'app-tong-hop-ket-qua',
  templateUrl: './tong-hop-ket-qua.component.html',
  styleUrls: ['./tong-hop-ket-qua.component.scss'],
  providers: [
...
export class TongHopKetQuaComponent implements
OnInit,
OnDestroy {
isLoading: boolean;
...
getbaocao() {
  ...
  const subscribe = this.service.getBaoCaoTHTCDN(filter).pipe(
    catchError(err => {
      return of(undefined);
    })).subscribe(res => {
      debugger;
...
exportExcel() {
  ...
  this.service.exportExcelTHKQ(filter).subscribe((response) => {
    if(response === undefined || response === null){
      ...
      link.download="Báo cáo tổng hợp kết quả.xlsx";
      link.click();
